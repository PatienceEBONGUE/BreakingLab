#!/bin/bash
## dependance : docker netstat grep at
...
start)
	...
	if docker ps | grep " $USER$" > /dev/null
	then
		IDrunning=$(docker ps | tr -s ' ' | grep " $3$"| cut -d ' ' -f 1)
	...
	then
		...
		END=$(date -d "now +$TIME min" +%H:%M)								## le timer ne marche pas
		echo "bash $(pwd)/proj-script.sh stop $PORT" | at $END 2> /dev/null		## action bien prise en compte mais ne ferme pas
		exit 0
	else
...
stop)
	...
	if docker ps | grep "$ID" > /dev/null 2> /dev/null	## verifie que l'ID donn̩existe bien (pas super utile mais pas genant) 
	then
		echo "\"$ID\""
